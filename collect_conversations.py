...
def get_message_history(session, fb_dtsg, my_id, friend_id, username, thread=False):
	...
	while True:
		...
		#insert message_dict into DB!
		#post_id = collection.insert_one(message_dict).inserted_id
		#print ("this is the unique id in the current MongoDB collection: " + str(post_id))
		...
		for message in message_list:
			...
			try:
				...
				message_collection['content'] = message_content
				#print (message_collection['content'])
			except KeyError:
			...
			message_collection['time_stamp'] = timeStamp


			# Type of device from which message was sent
			deviceType = ' '.join(message['source_tags'])
			if 'mobile' in deviceType:
				deviceType = 'mobile'
			else:
				deviceType = 'chat'
			message_collection['device_type'] = deviceType;
			...
			message_collection['sender'] = sender
			...
			#finished building message_collection
			#insert into mongoDB
			print ("inserted....")
			print (message_collection)
			collection.insert_one(message_collection)


			out = [sender, message_content.encode('utf8'), deviceType, timeStamp, coords]
...
for line in friend_file:
	...
	num_messages = get_message_history(session, fb_dtsg, int(my_id), uid, username)
